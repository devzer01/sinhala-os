...
class Disassembler(object):
    ...
    def read_instruction(self):
        ...
        intx = Instruction(address, code, mneumonic, *arguments)

#        if (mneumonic == 'JMP'):
#            self.index = int("0x" + arguments, 16)
    ...
    def disassemble(self, bytes, segment=0, offset=0, trap=0, quiet=0):
        ...
        while (self.index < len(self.bytes)):
           # print self.
           #print (self.read_integer())
           print (self.read_instruction())
...
x = Disassembler()
# x.bytes =
# file("bootsector.bin").read(512)
# while x.index <= len(x.bytes):
x.disassemble(file("readsector.bin").read(64))
